...
def get_lead(v_ego, ready, clusters, lead_msg, vLeads, Dists, Stds, dStds, low_speed_override=True):
  ...
  if cluster is not None:
    ...
    lead_dict = Cluster().get_RadarState_from_vision(lead_msg, v_ego, vLeads, Dists, Stds, dStds)
...
class RadarD():
  ...
  def update(self, sm, rr):
    ...
    if len(leads_v3) > 1:
      radarState.leadOne = get_lead(self.v_ego, self.ready, clusters, leads_v3[0], self.vLeads0, self.Dists0, self.vStds0, self.dStds0, low_speed_override=True)
      radarState.leadTwo = get_lead(self.v_ego, self.ready, clusters, leads_v3[1], self.vLeads1, self.Dists1, self.vStds1, self.dStds1, low_speed_override=False)
    return dat
